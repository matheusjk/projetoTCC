...
//    nome = chamaQuery[12].c_str();
  ...
  //  Serial.print("TELEMETRIA JSON: ");
    ...
    if(alertaEmail and lerSensorMQ() >= gasAviso){ //lerSensorMQ()){
     ...
     */
     ...
     session.login.user_domain = "";
      ...
      htmlMsg += "<p> Valor Gas Alerta: " + String(lerSensorMQ()) + " PPM </p>";
...
if((millis() - ultimoTempoThingSpeak) > tempoExecucaoThingSpeak){
  if(WiFi.status() == WL_CONNECTED){
     enviaThingSpeak(urlThingSpeak, secretKeyThingSpeak, temperatura, umidade, lerSensorMQ());  
  }
...
//        nome = retornaQuery();
      ...
      /* ==================================================================================================================================================== */
        ...
        sprintf(JSON, "{\"SENSORG\": %d, \"SENSORT\": %.2f, \"SENSORU\": %.2f, \"NOME\": \"%s\", \"IDUSUARIO\": %d }", lerSensorMQ(), temperatura, umidade, nome.c_str(), usuarioId);
//        Serial.print("CHIP ID ESP");
//        Serial.print(ESP.getChipId());
        Serial.println();
        Serial.print(JSON);
